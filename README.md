...
<img src="https://github.com/EmbarkStudios.png?size=48" width="32" height="32" alt="EmbarkStudios" align="left" /> **[cargo-about #319](https://github.com/EmbarkStudios/cargo-about/pull/319)** - terminate the appended license value with its closing quote in `synthesize_manifest` when the `[package]` table runs to end of file, so the synthesized manifest stays valid TOML with `package.license` intact, with the first unit test module in `resolution.rs` covering exact output, expression offset, and a TOML round-trip.

<img src="https://github.com/EmbarkStudios.png?size=48" width="32" height="32" alt="EmbarkStudios" align="left" /> **[cargo-about #320](https://github.com/EmbarkStudios/cargo-about/pull/320)** - rebase the insert position computed from the `&existing[pkg+5..]` subslice back to full-manifest coordinates in `synthesize_manifest`, fixing corrupted output and an `is_char_boundary` panic on multibyte CRLF manifests while keeping the reported offset at the expression start, with mid-file offset tests covering ASCII and multibyte TOML round-trips.

<img src="https://github.com/stellar.png?size=48" width="32" height="32" alt="Stellar" align="left" /> **[js-stellar-sdk #1725](https://github.com/stellar/js-stellar-sdk/pull/1725)** - docs pipeline now covers docs/migration pages: Starlight sidebar group, llms.txt and llms-full.txt bundles with strict link validation, wired through astro.config.mjs, build-llms.ts and doc-links.ts.
...
<img src="https://github.com/leo-aa88.png?size=48" width="32" height="32" alt="leo-aa88" align="left" /> **[reviewgate #172](https://github.com/leo-aa88/reviewgate/pull/172)** - corrects DESIGN.md section 10.14 Hunk entry state to match the shipped code-token rule from PR 144: each establishing context line must scan clean and carry a code token, with blank lines neutral, so docstring prose can no longer read as a normal code position.
